...
const handleRegister = async () => {
  ...
  try {
    ...
    if (profileFile.value) {
      const fileName = `${Date.now()}_${profileFile.value.name}`
    ...
    // 2. Register via API
    const { data: result, error: apiError } = await useFetch('/api/register', {
      method: 'POST',
      body: {
        ...form.value,
    ...
    if (apiError.value) {
      throw new Error(apiError.value.data?.statusMessage || 'การลงทะเบียนล้มเหลว')
    }
